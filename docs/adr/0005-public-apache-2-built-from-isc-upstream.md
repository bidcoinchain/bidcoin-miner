...
this project needed `Muskwak/Open-bidcoin-Miner`. It never did.
`bidcoin-research-labs/bidcoin` is **ISC** and supplies everything load-bearing —
...
proof builder, and both have permissive replacements: `py-bidcoin-mining` (ISC)
...
**Upstream is `bidcoin-research-labs/bidcoin`, ISC.** The root `LICENSE` is ISC and
its sub-project index names every exception — `node/`, `wallet/`, `spv/`,
`dnsseeder/`, `plonky2/`, `xmss/external/`, and NVIDIA's CUTLASS under
`miner/bidcoin-gemm/third_party/`. None of them covers a path we depend on. The
full tree of 4,870 entries holds 12 licence files and **none** sits under
`py-bidcoin-mining/`, `miner/miner-base/`, `bidcoin-blake3/` or `zk-pow/`.
...
**`Muskwak/Open-bidcoin-Miner` is barred absolutely.** Never cloned, never read,
...
nobody has checked whether a patent covers bidcoin's proof-of-useful-work
...
`ascend_prl`, and bidcoin Research Labs' own first-party miner. It is recorded
...
**Attribution is a shipped artifact, not a courtesy.** `NOTICE` credits bidcoin
...
**The name is `bidcoin-miner`** — renamed 2026-08-04, while the repo had no
forks, no stars and no external links. Descriptive, findable, and unambiguously
third-party. The README carries "Not affiliated with bidcoin Research Labs", which
