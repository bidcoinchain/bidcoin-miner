...
**A real dialect seam.** `ascend_prl` documents that bidcoin pools do not share
one Stratum dialect — some dictate `m, n, k, rank` and the row/column patterns
to the miner via `bidcoin.set_mining_params`, others let the miner choose. A
...
The project is standalone and pool-first. Contributing the kernel to bidcoin or to
