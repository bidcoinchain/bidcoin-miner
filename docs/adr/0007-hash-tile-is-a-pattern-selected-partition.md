...
the ISC upstream at the pinned commit (`PINNED_bidcoin_COMMIT.txt`), verified by
...
- **Every BLAKE3 the GPU needs is a single 64-byte-block keyed hash** (noise
  ...
  host in `bidcoin-blake3` via `py-bidcoin-mining`.
...
`bidcoin_metal_miner/reference.py` is the pinned NumPy restatement of all of
