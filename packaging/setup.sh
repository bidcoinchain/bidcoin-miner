...
# (pinned), and the py-bidcoin-mining Rust extension. Requires python3.12+ and
...
[ -n "$PY" ] || {
  echo "python >= 3.12 required (py-bidcoin-mining is abi3-py312)."
...
if [ ! -d bidcoin ]; then
  git clone --depth 1 https://github.com/bidcoin-research-labs/bidcoin.git
fi
if [ -f PINNED_bidcoin_COMMIT.txt ]; then
  WANT="$(cat PINNED_bidcoin_COMMIT.txt)"
  HAVE="$(git -C bidcoin rev-parse HEAD)"
  if [ "$WANT" != "$HAVE" ]; then
    ...
    if git -C bidcoin fetch --depth 1 origin "$WANT" &&
       git -C bidcoin checkout -q "$WANT"; then
      echo "bidcoin: checked out pinned commit $WANT"
    else
  ...
  git -C bidcoin rev-parse HEAD > PINNED_bidcoin_COMMIT.txt
fi

( cd bidcoin/py-bidcoin-mining && "$ROOT"/.venv/bin/maturin develop --release )
.venv/bin/python -c "import bidcoin_mining; print('bidcoin_mining OK', bidcoin_mining.__version__)"
