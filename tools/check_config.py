...
def run_wizard(cfg: str, answers: list[str], *extra: str):
    ...
    return subprocess.run(
        [sys.executable, "-m", "bidcoin_metal_miner.miner", "init", *extra],
        input="".join(a + "\n" for a in answers),
...
def check_precedence(cfg: str) -> bool:
    ...
    from bidcoin_metal_miner.miner import _apply_config, _explicit_dests, build_parser
    import bidcoin_metal_miner.miner as miner_mod
    warnings: list[str] = []
...
def check_bare_run(cfg: str) -> bool:
    ...
    env = dict(os.environ, PRL_CONFIG=cfg)
    proc = subprocess.Popen([sys.executable, "-m", "bidcoin_metal_miner.miner"],
...
def check_no_config_pointer() -> bool:
    ...
    import bidcoin_metal_miner.miner as miner_mod
    import bidcoin_metal_miner.wallet as wallet_mod
...
def no_wallet_left_behind():
    ...
    """
    from bidcoin_metal_miner import wallet as w
