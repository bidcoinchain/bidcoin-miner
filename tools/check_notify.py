...
from bidcoin_metal_miner.notify import Notifier  # noqa: E402


def check_unit() -> bool:
    import bidcoin_metal_miner.notify as mod
    ...
    try:
        ...
        n.send("bidcoin miner", 'total "1"')   # sent
        clock[0] = 1.0
        n.send("bidcoin miner", "total 2")     # coalesced away
        clock[0] = 5.0
        n.send("bidcoin miner", "total 3")     # sent
        off = Notifier(enabled=False, log=logs.append, now=lambda: clock[0])
        off.send("x", "y")                   # no-op
        time.sleep(0.3)
        assert len(calls) == 2, calls
        assert calls[0] == 'display notification "total \\"1\\"" with title "bidcoin miner"', calls
        assert not logs
...
def easy_target_hex() -> str:
    from bidcoin_metal_miner import reference as ref
