"""Differentially test bidcoin_metal_miner.wallet against independent oracles.
...
faith. It checks fresh random keys three ways:
  ...
  C. the witness program from upstream's authored coinbase fixture
     (bidcoin-gateway/tests/test_blockchain_utils.py) round-tripped through
     our codec.

Needs the dev-only dependency `pip install bitcoin-utils` (MIT) and the
upstream clone in ./bidcoin (setup.sh puts it there). Not part of the runtime,
...
from bidcoin_metal_miner import wallet  # noqa: E402
...
_GW = os.path.join(REPO, "bidcoin", "miner", "bidcoin-gateway", "src", "bidcoin_gateway",
...
def main(n_keys: int = 32) -> int:
    ...
    from bidcoin_metal_miner.selftest import UPSTREAM_FIXTURE_PROGRAM
